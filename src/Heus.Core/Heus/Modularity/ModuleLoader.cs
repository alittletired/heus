...
namespace Heus.Modularity
{
    internal class ModuleLoader
    {
        ...
        public ServiceModuleDescriptor[] LoadModules(IServiceCollection services, Type startupModuleType)
        {
        ...
        private List<ServiceModuleDescriptor> GetDescriptors(IServiceCollection services, Type startupModuleType)
        {
        ...
        protected virtual ServiceModuleDescriptor CreateModuleDescriptor(IServiceCollection services, Type moduleType,
            bool isLoadedAsPlugIn = false)
        {
            return new ServiceModuleDescriptor(moduleType, CreateAndRegisterModule(services, moduleType),
                isLoadedAsPlugIn);
        }
        ...
        protected virtual void SetDependencies(List<ServiceModuleDescriptor> modules, ServiceModuleDescriptor module)
        {
            foreach (var dependedModuleType in ServiceModuleHelper.FindDependedModuleTypes(module.Type))
            {
                ...
                {
                    throw new HeusException("Could not find a depended module " +
